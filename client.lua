Citizen.CreateThread(function()
    while QBCore.Functions.GetPlayerData().job == nil do Wait(10) end
end)
...
RegisterNetEvent("qb-truckrobbery:gruppeCard")
AddEventHandler("qb-truckrobbery:gruppeCard", function()
    ...
    if targetVehicle ~= 0 and GetHashKey("stockade") ==
        ...
        if aDist < 2.0 then
            ...
            TriggerEvent("qb-truckrobbery:AttemptHeist", targetVehicle)
        else
...
RegisterNetEvent('qb-truckrobbery:AttemptHeist')
AddEventHandler('qb-truckrobbery:AttemptHeist', function(veh)
    attempted = veh
    SetEntityAsMissionEntity(attempted, true, true)
    local plate = GetVehicleNumberPlateText(veh)
    local pedCo = GetEntityCoords(PlayerPedId())
    QBCore.Functions.Progressbar("unlockdoor_action", "Unlocking Vehicle",
                                 45000, false, true, {
        ...
        if not status then
            TriggerEvent("qb-truckrobbery:AllowHeist", veh)
            TriggerServerEvent("qb-truckrobbery:removeItem", "security_card_03", 1)
            TriggerServerEvent("qb-truckrobbery:PlateLog", plate)
        end
    end)
end)

RegisterNetEvent('qb-truckrobbery:AllowHeist')
AddEventHandler('qb-truckrobbery:AllowHeist', function(veh)
    TriggerEvent("qb-truckrobbery:AddPeds", attempted)
    SetVehicleDoorOpen(attempted, 2, 0, 0)
    SetVehicleDoorOpen(attempted, 3, 0, 0)
    TriggerEvent("qb-truckrobbery:PickupCash")
end)

RegisterNetEvent('qb-truckrobbery:AddPeds')
AddEventHandler('qb-truckrobbery:AddPeds', function(veh)
...
local pickup = false
RegisterNetEvent('qb-truckrobbery:PickupCash')
AddEventHandler('qb-truckrobbery:PickupCash', function()
    pickup = true
    TriggerEvent("qb-truckrobbery:PickupCashLoop")
    Wait(180000)
    pickup = false
end)

RegisterNetEvent('qb-truckrobbery:PickupCashLoop')
AddEventHandler('qb-truckrobbery:PickupCashLoop', function()
...
function pickUpCash()
    if not pickingup then
        ...
        while pickingup do
            ...
            if chance >= 50 then
                TriggerServerEvent('qb-truckrobbery:addItem', "security_card_01", 1)
            end

            for i = 1, #Config.Items, 1 do
                Wait(3000)
                TriggerServerEvent('qb-truckrobbery:addItem', Config.Items[i].name, Config.Items[i].count)
            end
            TriggerServerEvent('qb-truckrobbery:addMoney', Config.Money)
            pickingup = false
